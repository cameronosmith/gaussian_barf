...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        cam_lr = (1e-3 if c_iteration < s*3000 else 5e-4 if c_iteration < s*4000 else 1e-4 
                       if c_iteration<s*8000 else 5e-5 if c_iteration<s*10000 else 5e-6 if c_iteration<s*12000 else 0) if not args.no_anneal_cam_lr else 1e-3
        ...
        if iteration > args.start_cam_opt: 
            ...
            render_depth = render(viewpoint_cam, gaussians, pipe, background,pre_transf=transf,fov=focal_params,render_depth=True)["render"] # with pose gradients
        else: 
            render_pkg = render_(viewpoint_cam, gaussians, pipe, bg) # without (faster)
            render_depth = render(viewpoint_cam, gaussians, pipe, background,pre_transf=transf,fov=focal_params,render_depth=True)["render"] # with pose gradients
        render_depth=render_depth[0]

        #gt_flow=scene.scene_info.bwd_flows[cam_i]
        #gt_depth=scene.scene_info.gt_depths[cam_i]

        #res=gt_flow.shape[-2:]
        #uv = np.mgrid[0 : res[0], 0 : res[1]].astype(float).transpose(1, 2, 0)
        #uv = torch.from_numpy(np.flip(uv, axis=-1).copy()).long()
        #uv = uv / torch.tensor([res[1]-1, res[0]-1])  # uv in [0,1]
        #uv = uv[None].flatten(1,2).cuda()

        ## TODO set with focal params
        #norm_K=viewpoint_cam.norm_K.cuda() 

        #curr_cam,prev_cam=[lift_to_poses(transf_params[max(0,cam_i-j)]).inverse() for j in [0,1]]
        #ros_,rds_ = geometry.get_world_rays(uv,norm_K,None)
        #pose_flow = unhom(torch.einsum("bij,bkj->bki",norm_K[None], torch.einsum("bij,bkj->bki",prev_cam[None].inverse()@curr_cam[None],hom(rds_*render_depth.flatten(0,1)[None,:,None]))[...,:3]))-uv
        #pose_flow=pose_flow.permute(0,2,1).unflatten(-1,render_depth.shape[-2:])[0]

        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

        #flow_loss=(pose_flow-gt_flow).square().mean()*4e3

        #depth_loss=(render_depth-gt_depth[0]).square().mean()*5e0
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image))# + depth_loss + flow_loss
        loss.backward()
