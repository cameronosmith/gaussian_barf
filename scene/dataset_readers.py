...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    eval=True
    for colmap_model_index in reversed(range(9)):
        try:
            try:
                cameras_extrinsic_file = os.path.join(path, f"sparse/{colmap_model_index}", "images.bin")
                cameras_intrinsic_file = os.path.join(path, f"sparse/{colmap_model_index}", "cameras.bin")
                cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
                cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
                break
            except Exception:
                cameras_extrinsic_file = os.path.join(path, f"sparse/{colmap_model_index}", "images.txt")
                cameras_intrinsic_file = os.path.join(path, f"sparse/{colmap_model_index}", "cameras.txt")
    ...
    ply_path = os.path.join(path, f"sparse/{colmap_model_index}/points3D.ply")
    bin_path = os.path.join(path, f"sparse/{colmap_model_index}/points3D.bin")
    txt_path = os.path.join(path, f"sparse/{colmap_model_index}/points3D.txt")

    if os.path.exists(os.path.join(path, "dense/fused.ply")): 
        ply_path = os.path.join(path, "dense/fused.ply")
        print("Using DENSE colmap output")
